...
# Usage:
#   ./azure/deploy-frontend-app-service.sh <resource-group> <acr-name> [app-name] [image-tag]
#
# Example:
#   ./azure/deploy-frontend-app-service.sh my-rg myacr my-frontend latest
...
for f in "$SCRIPT_DIR/.env.azure" "$SCRIPT_DIR/env.azure"; do
  if [[ -f "$f" ]]; then
    echo "[deploy-frontend-app-service] Loading Azure config from: $f"
    set -a
